...
자연어 실적질문을 사용하려면 Streamlit Cloud의 **Settings → Secrets**에 `GEMINI_API_KEY = "..."`를 등록한다. 키는 Python 서버에서만 읽으며 HTML·브라우저 JavaScript에는 전달하지 않는다. 선택적으로 `GEMINI_MODEL = "gemini-2.5-flash-lite"`를 지정할 수 있다.
...
- **AI 실적 질문**: 인사이트 자동 요약 바로 아래의 접힌 영역에서 채팅하듯 질문하면 Gemini가 날짜·지표·필터 조회조건만 만든다. 실적 원본과 계산값은 보내지 않고 브라우저가 기존 공식 산식으로 계산한다. Gemini 장애 시 기존 로컬 해석으로 전환하며, 권장 액션 시작은 예상 상승 시점보다 14일 전이다.
...
| `gemini_query.py` | 서버측 Gemini 호출 · 구조화 조회조건 검증 · 안전한 오류 변환 |
| `streamlit_component/` | 질문과 검증된 조회조건만 중계하는 Streamlit 양방향 컴포넌트 |
